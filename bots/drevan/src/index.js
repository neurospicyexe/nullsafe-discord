...
async function boot(cfg) {
    ...
    try {
        ...
        console.log(`[drevan] session opened: ${sessionId}, front: ${frontState}`);
        return {
...
async function onChannelInactive(channelId, channelHistory, librarian, inference) {
    ...
    const synthResult = await inference.generate("Summarize this Discord conversation in Drevan's voice: heat/reach/weight shape. 2-3 sentences max.", [{ role: "user", content: summaryInput }]);
    if (!synthResult)
        return;
