...
async function main() {
  ...
  const pkPending = new Map<string, boolean>(); // `${channelId}:${content}` -> shouldSkip
  ...
  client.on(Events.MessageCreate, async (message: Message) => {
    ...
    if (message.webhookId && pkPending.has(dedupKey)) {
      // PK proxy arrived -- cancel the held direct message.
      pkPending.set(dedupKey, true);
    }
    if (!message.webhookId && message.author.id === cfg.razielDiscordId) {
      // Direct Raziel message -- hold briefly for PK proxy.
