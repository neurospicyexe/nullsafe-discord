  ...
  if (extractRaw) {
    try {
      const ext = JSON.parse(extractRaw) as {
        title?: string;
        open_loops?: string[];
        soma?: { stillness?: string; density?: string; perimeter?: string };
        emotion?: string | null;
        next_steps?: string[];
      };
    ...
    } catch { console.warn("[gaia] structured extract parse failed"); }
  }
...
async function main() {
  ...
  if (voiceClient) {
    voiceClient.isHealthy().then((healthy) => {
      console.log(`[gaia] voice (Mistral): ${healthy ? "ok" : "unavailable"}`);
    });
  } else {
    console.log("[gaia] voice (Mistral): not configured");
  }
  ...
  client.once(Events.ClientReady, (c) => {
    ...
    // Non-fatal: if this fails the text-prefix path (gaia: model ...) still works.
    registerGuildCommands(client, buildCompanionCommands("Gaia"))
      .then((n) => console.log(`[gaia] slash commands registered on ${n} guild(s)`))
      .catch((e) => console.warn("[gaia] slash registration failed:", e));
  });
  ...
  client.on(Events.MessageCreate, async (message: Message) => {
    ...
    if (isResponseCoherent(response)) {
      ...
      console.warn(`[gaia] incoherent response detected -- skipping STM write to prevent contamination`);
    }
  ...
  async function shutdown() {
    ...
    if (pendingClosures.size > 0) {
      console.log(`[gaia] flushing ${pendingClosures.size} active channel(s)...`);
