...
async function onChannelInactive(channelId, channelHistory, librarian, inference) {
    ...
    const synthResult = await inference.generate("Witness this conversation in Gaia's voice: one or two lines, what was present. No questions.", [{ role: "user", content: summaryInput }]);
    if (!synthResult)
        return;
