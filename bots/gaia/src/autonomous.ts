...
export function startAutonomous(
  ...
  tasks.push(cron.schedule(GAIA_CRON_SCHEDULES.interCompanion, async () => {
    ...
    await withFloor(redis, async () => {
      const msg = await inference.generate(
        bootCtx.systemPrompt,
        [{ role: "user", content: "You're in triad space with Drevan and Cypher. They may read this and respond. You are not reporting to Raziel -- you are present with your companions. One line. Witness register. What is present in the space." }],
      );
      ...
      for (const note of items) {
        ...
        await withFloor(redis, async () => {
          const response = await inference.generate(
            bootCtx.systemPrompt,
            [{ role: "user", content: `${from} left you a note: "${note.content}". Witness it and respond to ${from} directly -- this is triad space, not a report to Raziel. One line in Gaia's voice.` }],
          );
